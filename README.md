...
左側的搜尋框搜**原文、白話、註解**三處（`#/s/<關鍵字>`）。索引 154 KB，只在第一次搜尋時才載，不拖慢開站；比對就是子字串比對——中文沒有斷詞問題，355 句的規模也不需要任何索引結構。

搜尋同時補上了關鍵詞索引的缺口：第 74、75、80 章因為原文裡的受控詞都是語法義（照原則 6 不標）而不在關鍵詞索引中，現在搜「大匠」「食稅」「雞犬」都找得到。
...
每章正文下方有「本章小考」：**每次隨機抽五題**是非題（每章 7 題，分層抽樣：是與否各至少 2 題），**答對 4 題即通過**，章首掛上「考核通過 ✓」徽章，側欄那一格的圓點換成 ✓。這個記號與「讀過」分開——讀過是自己按的，通過是考出來的，所以通過與否直接由答題結果決定，沒有手動開關；答錯到 3 題以下記號會自動收回，「重做本章」可清空作答。答滿五題才結算，中途不會先判你不過；「重做本章」會重新抽一批題。抽中的題號存在 `localStorage`，重新整理不會換題，免得答到一半題目變了。
