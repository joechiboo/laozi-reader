...
data/
  ...
  quiz.json             小測驗題庫（是非題），每題掛一個句 id 指回原文
...
`#/quiz` 是十題是非題的小測驗，考的都是最容易讀反的地方（無為不是不做、天地不仁不是冷血、道法自然上面沒有老闆）。答完才顯示解說，並附一個指回原文的句 id 連結——題目與章資料是分開的檔，所以 `validate.py` 會檢查每題的 `ref` 是否還指得到句，免得改了斷句之後題目變孤兒。作答存在 `localStorage`，不上傳、不跨裝置。
