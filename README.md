...
data/
  ...
  quiz/NNN.json         本章小考題庫，一章至少五題；ref 必須指到本章的句
...
每章正文下方有「本章小考」：**每次隨機抽五題**是非題（題庫可以比五題多，第 1 章目前 8 題），**答對 4 題即通過**，章首掛上「考核通過 ✓」徽章，側欄那一格的圓點換成 ✓。這個記號與「讀過」分開——讀過是自己按的，通過是考出來的，所以通過與否直接由答題結果決定，沒有手動開關；答錯到 3 題以下記號會自動收回，「重做本章」可清空作答。答滿五題才結算，中途不會先判你不過；「重做本章」會重新抽一批題。抽中的題號存在 `localStorage`，重新整理不會換題，免得答到一半題目變了。
